    ...
    mbFixScale(bFixScale)
{
    ...
    vector<MapPoint*> vpKeyFrameMP1 = pKF1->GetMapPointMatches();  // KF1 MP 观测集
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        ...
        if(vpMatched12[i1])
        {
            ...
            const float sigmaSquare1 = pKF1->mvLevelSigma2[kp1.octave];  // KF1 KP 尺度平方
            const float sigmaSquare2 = pKF2->mvLevelSigma2[kp2.octave];  // KF2 KP 尺度平方
...
void Sim3Solver::CheckInliers()
{
    ...
    {
        cv::Mat dist1 = mvP1im1[i]-vP2im1[i];  // KF1 MP 与 KF2 MP 在 KF1 投影坐标误差
        cv::Mat dist2 = vP1im2[i]-mvP2im2[i];  // KF1 MP 与 KF2 MP 在 KF2 投影坐标误差
...
/// @brief 计算世界系下地图点按给定变换矩阵、内参投影的像素坐标  这里的"世界系"可能是另一个 KF
/// @param vP3Dw 地图点 tw
...
/// @brief Sim3 求解  理论上可以进行 n 点匹配，实际只传入 3 个点对  结果存放于成员变量
...
void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2)
{
    ...
    cv::Mat Pr1(P1.size(),P1.type());  // KF1 三点去中心坐标  3*3  Relative coordinates to centroid (set 1)
    cv::Mat Pr2(P2.size(),P2.type());  // KF2 三点去中心坐标  3*3  Relative coordinates to centroid (set 2)
    ...
    cv::eigen(N,eval,evec); // 特征值分解  evec[0] is the quaternion of the desired rotation
    ...
    vec = 2*ang*vec/norm(vec); // 角轴，注意四元数转角是实际转角的一半  Angle-axis representation. quaternion angle is the half

    mR12i.create(3,3,P1.type());  // 旋转矩阵

    cv::Rodrigues(vec,mR12i);  // 罗德里格斯公式 computes the rotation matrix from angle-axis
