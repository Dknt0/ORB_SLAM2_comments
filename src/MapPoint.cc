    ...
    mpMap(pMap)
{
    ...
    const float levelScaleFactor =  pFrame->mvScaleFactors[level];  // 金字塔尺度对应比例 >1
    const int nLevels = pFrame->mnScaleLevels;  // 金字塔总层数

    mfMaxDistance = dist*levelScaleFactor;  // 最远距离为在 0 层检测到特征点时对应的距离
    mfMinDistance = mfMaxDistance/pFrame->mvScaleFactors[nLevels-1];  // 最近距离为在最高层检测到特征点时对应的距离
...
void MapPoint::SetBadFlag()
{
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        pKF->EraseMapPointMatch(mit->second);  // 关键帧内 按照关键点索引清除地图点匹配
    }
...
void MapPoint::Replace(MapPoint* pMP)
{
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        {
            pKF->ReplaceMapPointMatch(mit->second, pMP);  // 将旧地图点对应关键点的匹配设置为新点
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    const int level = pRefKF->mvKeysUn[observations[pRefKF]].octave;  // 对应关键帧关键点金字塔尺度
...

    {
        ...
        mfMinDistance = mfMaxDistance/pRefKF->mvScaleFactors[nLevels-1];  // 最近尺度无关距离
