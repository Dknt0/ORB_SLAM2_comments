...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    const vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();  // 当前 KF MP
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    if(mbMonocular)
        nn=20;
    const vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);  // 近邻 KF
    ...
    const float ratioFactor = 1.5f*mpCurrentKeyFrame->mfScaleFactor;  // 尺度比例阈值
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        {
            const float medianDepthKF2 = pKF2->ComputeSceneMedianDepth(2);  // 场景深度中位数
        ...
        vector<pair<size_t,size_t> > vMatchedIndices;  // KP 匹配 v<p<KF1 idx, KF2 idx>>  结果中不包含当前 KF 中已经匹配 MP 的 KP
        /* 三角化匹配 */
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const float kp1_ur=mpCurrentKeyFrame->mvuRight[idx1];  // KF1 KP 右点坐标
            ...
            /* 计算观测方向余弦 */
            // 归一化平面坐标
            cv::Mat xn1 = (cv::Mat_<float>(3,1) << (kp1.pt.x-cx1)*invfx1, (kp1.pt.y-cy1)*invfy1, 1.0);  // KF1 KP 归一化平面坐标
            cv::Mat xn2 = (cv::Mat_<float>(3,1) << (kp2.pt.x-cx2)*invfx2, (kp2.pt.y-cy2)*invfy2, 1.0);  // KF2 KP 归一化平面坐标

            cv::Mat ray1 = Rwc1*xn1;  // KF1 KP 方向向量  世界系
            cv::Mat ray2 = Rwc2*xn2;  // KF2 KP 方向向量  世界系
            const float cosParallaxRays = ray1.dot(ray2)/(cv::norm(ray1)*cv::norm(ray2));  // KF1 KF2 KP 观测方向余弦

            /* 计算双目视差余弦 */
            // 这里的视差余弦是指以双目基线为对直角边，深度为侧直角边构成直角三角形顶角的余弦值
            float cosParallaxStereo = cosParallaxRays+1;  // 双目视差余弦，初值给一个比较大的  这个值越大，代表相对深度越大
            ...
            // 优先使用当前 KF 的双目视差余弦，如果两个 KP 都不是双目点，则双目视差余弦大于 1
            ...
            if(cosParallaxRays<cosParallaxStereo && cosParallaxRays>0 && (bStereo1 || bStereo2 || cosParallaxRays<0.9998))
            {
                ...
                // 3. 其中一个 KP 为双目点或观测方向余弦小于阈值，对应观测方向角度大于 1.14 度，保证三角化能成功

                // 这里第三个条件中为什么考虑 KF 是否为双目点？既然是三角化，那和相机类型应该是没有关系的
            ...
            float z1 = Rcw1.row(2).dot(x3Dt)+tcw1.at<float>(2);  // 相对于 KF1 z 坐标
            if(z1<=0)
                continue;
            ...
            const float &sigmaSquare1 = mpCurrentKeyFrame->mvLevelSigma2[kp1.octave];  // KP 金字塔比例平方 >1
            const float x1 = Rcw1.row(0).dot(x3Dt)+tcw1.at<float>(0);  // KF1 归一化坐标 x
            const float y1 = Rcw1.row(1).dot(x3Dt)+tcw1.at<float>(1);  // KF2 归一化坐标 x
            ...
            const float ratioOctave = mpCurrentKeyFrame->mvScaleFactors[kp1.octave]/pKF2->mvScaleFactors[kp2.octave];  // 尺度比例
...
void LocalMapping::SearchInNeighbors()
{
    ...
    if(mbMonocular)
        nn=20;
    const vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);  // 近邻 KF 集
    ...
    for(vector<KeyFrame*>::const_iterator vit=vpNeighKFs.begin(), vend=vpNeighKFs.end(); vit!=vend; vit++)
    {
        ...
        const vector<KeyFrame*> vpSecondNeighKFs = pKFi->GetBestCovisibilityKeyFrames(5);  // 近邻 KF 的近邻
...
void LocalMapping::KeyFrameCulling()
{
    ...
    vector<KeyFrame*> vpLocalKeyFrames = mpCurrentKeyFrame->GetVectorCovisibleKeyFrames();  // 有序共视关键帧向量
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        if(pKF->mnId==0)
            continue;
        const vector<MapPoint*> vpMapPoints = pKF->GetMapPointMatches();  // 共视 KF MP 观测
            ...
            if(pMP)
            {
                if(!pMP->isBad())
                {
                    ...
                    if(pMP->Observations()>thObs)
                    {
                        ...
                        const map<KeyFrame*, size_t> observations = pMP->GetObservations();  // 当前 MP 观测映射
                        int nObs=0;
                        for(map<KeyFrame*, size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
                        {
                            ...
                            if(pKFi==pKF)
                                continue;
                            const int &scaleLeveli = pKFi->mvKeysUn[mit->second].octave;  // 在其他关键帧中的尺度
