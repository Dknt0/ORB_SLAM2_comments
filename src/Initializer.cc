...
void Initializer::SampleForRansec() {
  ...
  // RANSAC 最小样本 - 8 对
  mvSets =
      std::vector<std::vector<size_t>>(mMaxIterations, vector<size_t>(8, 0));
...
/// @param H21 单应矩阵 输出
void Initializer::FindHomography(std::vector<bool> &vbMatchesInliers,
  ...
  for (int it = 0; it < mMaxIterations; it++) {
    ...
    currentScore =
        CheckHomography(H21i, H12i, vbCurrentInliers, mSigma);  // 计算得分
...
float Initializer::CheckHomography(const cv::Mat &H21, const cv::Mat &H12,
  ...
  for (int i = 0; i < N; i++) {
    ...
    const float squareDist1 = (u1 - u2in1) * (u1 - u2in1) +
                              (v1 - v2in1) * (v1 - v2in1);  // 重投影误差平方
...
bool Initializer::ReconstructH(vector<bool> &vbMatchesInliers, cv::Mat &H21,
                               ...
                               int minTriangulated) {
  // 参考论文 Faugeras et al. Motion and structure from motion in a piecewise
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score,
  ...
  for (int it = 0; it < mMaxIterations; it++) {
    ...
    currentScore =
        CheckFundamental(F21i, vbCurrentInliers, mSigma);  // 计算得分
