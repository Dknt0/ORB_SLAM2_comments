...
namespace ORB_SLAM2
{
          ...
          mnLastRelocFrameId(0)
    {
        ...
        {
            mThDepth = mbf*(float)fSettings["ThDepth"]/fx; // 配置文件中 ThDepth 是基线的倍数
                                    ...
                                    double timestamp)
    {
        ...
        // 用户可以传入 uint16 类型的深度图，或 float 深度图，前者通常需要乘深度系数，后者像素中直接存放深度
        ...
        if(mState==NOT_INITIALIZED)
        {
            ...
            if (!mbOnlyTracking) {
                ...
                if (OK == mState) {
                    ...
                    // 如果速度信息缺失，或刚刚完成了重定位，根据参考 KF 追踪，否则根据速度模型追踪
                    if (mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId+2) {
                        bOK = TrackReferenceKeyFrame();  // 根据参考关键帧跟踪
                    } else {
                        bOK = TrackWithMotionModel();  // 根据匀速模型跟踪速度更快
                        if(!bOK)
                            bOK = TrackReferenceKeyFrame();  // 如果速度模型追踪失败，根据参考关键帧跟踪
                    }
                } else {
                    // SLAM 模式，追踪失败
            ...
            // 如果运动模型或参考关键帧追踪成功，则进一步追踪局部地图，寻找 F 及其 MP 与 LocalMap 的匹配关系
            if(!mbOnlyTracking)
            {
                // SLAM 模式  追踪成功
                if(bOK)
            ...
            if(bOK)
            {
                ...
                if(!mLastFrame.mTcw.empty())
                {
                    ...
                    mLastFrame.GetRotationInverse().copyTo(LastTwc.rowRange(0,3).colRange(0,3));  // Rwcl
                    mLastFrame.GetCameraCenter().copyTo(LastTwc.rowRange(0,3).col(3));  // twcl
                    mVelocity = mCurrentFrame.mTcw*LastTwc;  // Tcccl 两帧间的相对位姿态，并不是速度
                }
                else
                ...
                for(int i=0; i<mCurrentFrame.N; i++)
                {
                    ...
                    if(pMP)
                        ...
                        if(pMP->Observations()<1)
                        {
                            ...
                            mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);  // 舍弃 MP 观测
                        }
                }
                ...
                // 遍历当前 F MP 观测，如果观测为外点，舍弃之，防止其在下一次追踪中被使用
                for(int i=0; i<mCurrentFrame.N; i++)
                {
                    if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
    ...
    void Tracking::StereoInitialization()
    {
        ...
        if(mCurrentFrame.N>500)
        {
            ...
            // Create KeyFrame
            // 创建 KF
            KeyFrame* pKFini = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB);  // 从 mCurrentFrame 创建初始 KF
            ...
            mvpLocalMapPoints=mpMap->GetAllMapPoints();  // 初始 KF 深度恢复的所有 MP 放入局部地图
    ...
    void Tracking::CreateInitialMapMonocular()
    {
        ...
        // 如果深度中位数为负，或追踪到的有效地图点数小于 100，认为初始化错误，重置 Tracking
