...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        float r = RadiusByViewingCos(pMP->mTrackViewCos);  // 窗口半径 窗口范围由视角余弦、特征尺度确定
        ...
        const vector<size_t> vIndices =
                F.GetFeaturesInArea(pMP->mTrackProjX,pMP->mTrackProjY,r*F.mvScaleFactors[nPredictedLevel],nPredictedLevel-1,nPredictedLevel);  // 候选关键点序号集 地图点投影附近的关键点，尺度与预测尺度相同或小一个级别
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            // 如果这个关键点有地图点观测，且那个地图点的观测数大于 0，忽略   即，忽略已经观测到有效 MP 的 KP
            ...
            const int dist = DescriptorDistance(MPdescriptor,d);  // 地图点与特征点汉明距离
        ...
        /* 3.匹配对筛选，如果匹配距离大于阈值，或最佳、次佳匹配点来自不同层，抛弃匹配对，否则将 MP 添加到 F 观测集 */
...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    set<MapPoint*> spAlreadyFound(vpMatched.begin(), vpMatched.end());  // 已匹配的地图点集合
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        const float maxDistance = pMP->GetMaxDistanceInvariance();  // 最大尺度无关距离*1.2
        const float minDistance = pMP->GetMinDistanceInvariance();  // 最小尺度无关距离*0.8
        cv::Mat PO = p3Dw-Ow;  // 候选地图点相对于关键帧位置在世界系下的表示   其实和 p3Dc 模长是一样的
        ...
        // Search in a radius
        const float radius = th*pKF->mvScaleFactors[nPredictedLevel];  // 由预测尺度确定窗口半径

        const vector<size_t> vIndices = pKF->GetFeaturesInArea(u,v,radius);  // 投影点窗口范围内的候选关键点
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    // 旋转直方图，用于检查旋转一致性
    vector<int> rotHist[HISTO_LENGTH];  // 旋转直方图  [直方图列](vector<特征点序号>)
    ...
    const cv::Mat Rcw = CurrentFrame.mTcw.rowRange(0,3).colRange(0,3);  // 当前帧姿态 Rcw
    const cv::Mat tcw = CurrentFrame.mTcw.rowRange(0,3).col(3);  // 当前帧位置 tcw

    const cv::Mat twc = -Rcw.t()*tcw;  // 当前帧位置 twc

    const cv::Mat Rlw = LastFrame.mTcw.rowRange(0,3).colRange(0,3);  // 上一帧姿态 Rlw
    const cv::Mat tlw = LastFrame.mTcw.rowRange(0,3).col(3);  // 上一帧位置 tlw

    const cv::Mat tlc = Rlw*twc+tlw;  // 当前帧相对于上一帧位置 tlc

    const bool bForward = (tlc.at<float>(2) > CurrentFrame.mb) && !bMono;  // 前进标志位  位移 z 大于 0 时  单目无效
    const bool bBackward = (-tlc.at<float>(2) > CurrentFrame.mb) && !bMono;  // 后退标志位  位移 z 小于 0 时  单目无效
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        float u = CurrentFrame.fx*xc*invzc+CurrentFrame.cx;  // 地图点投影当前帧像素平面坐标 u
        float v = CurrentFrame.fy*yc*invzc+CurrentFrame.cy;  // 地图点投影当前帧像素平面坐标 v
        ...
        int nLastOctave = LastFrame.mvKeys[i].octave;  // 地图点在上一帧内特征点尺度级别

        // Search in a window. Size depends on scale
        float radius = th*CurrentFrame.mvScaleFactors[nLastOctave];  // 窗口范围由特征尺度决定

        vector<size_t> vIndices2;  // 上一帧的地图点在当前帧中候选关键点向量  关键点序号
        ...
        {
            float rot = LastFrame.mvKeysUn[i].angle-CurrentFrame.mvKeysUn[bestIdx2].angle;  // 上一帧关键点相对于当前帧特征旋转角度
            // 角度归一化  0~360
            if(rot<0.0)
                rot+=360.0f;
            // 直方图预留了 30 列，但这里最大列号 round(360 / 30) = 12，貌似只有 0~12 列会有点落入?
    ...
    if(mbCheckOrientation)
    {
        ...
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);  // 寻找直方图中前三大的列标号
        ...
        for(int i=0; i<HISTO_LENGTH; i++)
        {
            ...
            if(i!=ind1 && i!=ind2 && i!=ind3)
            {
                ...
                {
                    CurrentFrame.mvpMapPoints[rotHist[i][j]]=static_cast<MapPoint*>(NULL);  // 清除观测
                    nmatches--;  // 匹配数-1
                }
            }
        }
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint*> &sAlreadyFound, const float th , const int ORBdist)
{
    ...
    const vector<MapPoint*> vpMPs = pKF->GetMapPointMatches();  // 获取关键帧观测地图点集
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad() && !sAlreadyFound.count(pMP))
            {
                ...
                int nPredictedLevel = pMP->PredictScale(dist3D,&CurrentFrame);  // 预测尺度

                // Search in a window
                const float radius = th*CurrentFrame.mvScaleFactors[nPredictedLevel];  // 窗口范围由预测特征尺度决定

                const vector<size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(u, v, radius, nPredictedLevel-1, nPredictedLevel+1);  // 候选关键点序号集 地图点投影附近的关键点，尺度与预测尺度相差不超过1
                ...
                /* 4.如果最佳匹配距离小于阈值，为 KP 记录这个 MP，添加 KP 对应关系到旋转直方图中 */
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();  // 关键帧 1 匹配地图点集
    const int N1 = vpMapPoints1.size();  // 关键帧 1 关键点数

    const vector<MapPoint*> vpMapPoints2 = pKF2->GetMapPointMatches();  // 关键帧 2 匹配地图点集
    ...
    for(int i=0; i<N1; i++)
    {
        ...
        if(pMP)
        {
            ...
            int idx2 = pMP->GetIndexInKeyFrame(pKF2);  // 查询地图点在关键帧 2 中关键点索引
            if(idx2>=0 && idx2<N2)
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        const int nPredictedLevel = pMP->PredictScale(dist3D,pKF2);  // 预测金字塔尺度级别

        // Search in a radius
        const float radius = th*pKF2->mvScaleFactors[nPredictedLevel];  // 依据尺度级别确定窗口半径

        const vector<size_t> vIndices = pKF2->GetFeaturesInArea(u,v,radius);  // 获取 KF2 投影点窗口范围内候选关键点
    ...
    for(int i2=0; i2<N2; i2++)
    {
        ...
        // Compute predicted octave
        const int nPredictedLevel = pMP->PredictScale(dist3D,pKF1);  // 预测金字塔尺度级别

        // Search in a radius of 2.5*sigma(ScaleLevel)
        const float radius = th*pKF1->mvScaleFactors[nPredictedLevel];

        const vector<size_t> vIndices = pKF1->GetFeaturesInArea(u,v,radius);  // 获取 KF1 投影点窗口范围内候选关键点
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        if(idx2>=0)
        {
            int idx1 = vnMatch2[idx2];  // KF2 中的这个关键点匹配回 KF1 中的关键序号  和 i1 应该相等
            if(idx1==i1)
            {
                // 如果相等，代表匹配一致
...
{
    const vector<MapPoint*> vpMapPointsKF = pKF->GetMapPointMatches();  // 关键帧地图点观测集 const

    vpMapPointMatches = vector<MapPoint*>(F.N, static_cast<MapPoint*>(NULL));  // 清空帧地图点匹配集

    const DBoW2::FeatureVector &vFeatVecKF = pKF->mFeatVec;  // KF 视觉特征向量 const   map<节点id, 关键点序号集 vector<int>>
    ...
    DBoW2::FeatureVector::const_iterator KFit = vFeatVecKF.begin();  // KF 特征向量迭代器
    DBoW2::FeatureVector::const_iterator Fit = F.mFeatVec.begin();  // F 特征向量迭代器
    DBoW2::FeatureVector::const_iterator KFend = vFeatVecKF.end();  // KF 特征向量末尾迭代器
    DBoW2::FeatureVector::const_iterator Fend = F.mFeatVec.end();  // F 特征向量末尾迭代器

    // 遍历关键帧特征向量与帧特征向量，在两帧位于相同视觉词汇节点的关键点中，寻找匹配  视觉词汇约束下的暴力匹配
    while(KFit != KFend && Fit != Fend)
    {
        // 如果位于同一个视觉词汇节点
        if(KFit->first == Fit->first)
        {
            const vector<unsigned int> vIndicesKF = KFit->second;  // KF 候选关键点序号集
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                const cv::Mat &dKF= pKF->mDescriptors.row(realIdxKF);  // KF 关键点描述子
                ...
                if(bestDist1<=TH_LOW)
                {
                    ...
                    if(static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2))
                    {
                        ...
                        const cv::KeyPoint &kp = pKF->mvKeysUn[realIdxKF];  // 关键帧关键点  const
    ...
    if(mbCheckOrientation)
    {
        ...
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);  // 寻找直方图前三大的列序号
        ...
        for(int i=0; i<HISTO_LENGTH; i++)
        {
            ...
            for(size_t j=0, jend=rotHist[i].size(); j<jend; j++)
            {
                vpMapPointMatches[rotHist[i][j]]=static_cast<MapPoint*>(NULL);  // 清除匹配
                nmatches--;
            }
        }
    }
...
int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12)
{
    ...
    const vector<MapPoint*> vpMapPoints1 = pKF1->GetMapPointMatches();  // KF1 地图点
    ...
    const vector<MapPoint*> vpMapPoints2 = pKF2->GetMapPointMatches();  // KF2 地图点
    const cv::Mat &Descriptors2 = pKF2->mDescriptors;  // KF2 关键点描述子

    vpMatches12 = vector<MapPoint*>(vpMapPoints1.size(),static_cast<MapPoint*>(NULL));
    vector<bool> vbMatched2(vpMapPoints2.size(),false);  // KF2 地图点已被匹配  标志位
    ...
    DBoW2::FeatureVector::const_iterator f1it = vFeatVec1.begin();  // KF1 视觉特征向量迭代器
    DBoW2::FeatureVector::const_iterator f2it = vFeatVec2.begin();  // KF2 视觉特征向量迭代器
    DBoW2::FeatureVector::const_iterator f1end = vFeatVec1.end();
    DBoW2::FeatureVector::const_iterator f2end = vFeatVec2.end();

    // 遍历 KF1 视觉特征向量与 KF2 视觉特征向量，在两帧位于相同视觉词汇节点的关键点中，寻找匹配  视觉词汇约束下的暴力匹配
        ...
        if(f1it->first == f2it->first)
        {
            ...
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                if(bestDist1<TH_LOW)
                {
                    ...
                    if(static_cast<float>(bestDist1)<mfNNratio*static_cast<float>(bestDist2))
                    {
                        vpMatches12[idx1]=vpMapPoints2[bestIdx2];  // 为 KF1 记录这个地图点
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    vector<int> vMatchedDistance(F2.mvKeysUn.size(),INT_MAX);  // 匹配距离  F2 KP 索引
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,vbPrevMatched[i1].y, windowSize,level1,level1);  // 获取 F1 KP 在 F2 预匹配位置附近的候选关键点
...
{    
    ...
    const float ex =pKF2->fx*C2.at<float>(0)*invz+pKF2->cx;  // KF1 在 KF2 图像平面投影点  极点  u
    const float ey =pKF2->fy*C2.at<float>(1)*invz+pKF2->cy;  // KF1 在 KF2 图像平面投影点  极点  v
    ...
    vector<int> vMatches12(pKF1->N,-1);  // KF1 到 KF2 的匹配  按照 KF1 KP 索引  记录 KF2 特征点序号
...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    ...
    // Set of MapPoints already found in the KeyFrame
    const set<MapPoint*> spAlreadyFound = pKF->GetMapPoints();  // 关键帧地图点观测集合
    ...
    for(int iMP=0; iMP<nPoints; iMP++)
    {
        ...
        // Depth must be inside the scale pyramid of the image
        const float maxDistance = pMP->GetMaxDistanceInvariance();  // 最大尺度无关距离*1.2
        const float minDistance = pMP->GetMinDistanceInvariance();  // 最小尺度无关距离*0.
        cv::Mat PO = p3Dw-Ow;
        const float dist3D = cv::norm(PO);  // 距离
        ...
        // If there is already a MapPoint replace otherwise add new measurement
        // 如果已经有地图点观测，将旧地图点放入替换向量  如果还没有观测，则添加新点观测
        // 最佳匹配小于低阈值
        if(bestDist<=TH_LOW)
        {
            MapPoint* pMPinKF = pKF->GetMapPoint(bestIdx);  // 旧地图点
...
int ORBmatcher::DescriptorDistance(const cv::Mat &a, const cv::Mat &b)
{
    ...
    for(int i=0; i<8; i++, pa++, pb++)
    {
        ...
        dist += (((v + (v >> 4)) & 0xF0F0F0F) * 0x1010101) >> 24;  // 0x0F0F0F0F = 00001111 00001111 00001111 00001111
    }
...
/// @brief 特征匹配极线距离检查   检查 KP2 与 KP1 按照基础矩阵 F12 投影 到 KF2 图像平面极线的像素距离是否小于误差阈值
/// @param kp1 关键帧 1 关键点
/// @param kp2 关键帧 2 关键点
/// @param F12 基本矩阵
/// @param pKF2 关键帧 2
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,const cv::KeyPoint &kp2,const cv::Mat &F12,const KeyFrame* pKF2)
{
    ...
    const float num = a*kp2.pt.x+b*kp2.pt.y+c;  // 如果 KP2 位于极线上，则等于 0
