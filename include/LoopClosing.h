...
class LoopClosing {
 public:
  typedef pair<set<KeyFrame*>, int>
      ConsistentGroup;  // 共视组  pair<共视 KF 集合, 累积一致性>
                        // 共视组累积一致性指包含相同 KF 的共视组的数量

  // 下述内存申请器用于为 Eigen 矩阵申请对齐的内存，加速运算

  typedef map<KeyFrame*, g2o::Sim3, std::less<KeyFrame*>,
              Eigen::aligned_allocator<std::pair<KeyFrame* const, g2o::Sim3> > >
      KeyFrameAndPose;  // 位姿映射  map<KF, Sim3, 比较器, 内存申请器>
 ...
 protected:
  ...
  std::vector<ConsistentGroup>
      mvConsistentGroups;  // 历史共视组集  仅在回环检测中使用  vector<pair<共视
                           // KF 集合, 与其他组的共视数量>>  数据存储策略？
  std::vector<KeyFrame*>
      mvpEnoughConsistentCandidates;  // 满足一致性阈值的候选 KF 集

  /* 当前 KF 回环相关 */
  std::vector<KeyFrame*>
      mvpCurrentConnectedKFs;  // 当前 KF 局部范围内 KF  包括当前 KF 及其近邻 KF
  std::vector<MapPoint*>
      mvpCurrentMatchedPoints;  // 当前 KF 对回环匹配 KF MP 匹配
  std::vector<MapPoint*>
      mvpLoopMapPoints;  // 回环局部 MP  匹配 KF 局部范围内 MP
