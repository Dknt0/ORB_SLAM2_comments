...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    // 检查距离是否在尺度无关范围内
    const float maxDistance = pMP->GetMaxDistanceInvariance();  // 最大尺度不变距离*1.2
    const float minDistance = pMP->GetMinDistanceInvariance();  // 最小尺度不变距离*0.8
...
void Frame::ComputeStereoMatches()
{
    ...
    const int thOrbDist = (ORBmatcher::TH_HIGH+ORBmatcher::TH_LOW)/2;  // ORB 匹配汉明距离阈值
    ...
    vector<vector<size_t> > vRowIndices(nRows,vector<size_t>());  // 候选右图关键点  左图行号<候选特征点序号<右图特征点序号>>
    ...
    for(int iR=0; iR<Nr; iR++)
    {
        ...
        const float r = 2.0f*mvScaleFactors[mvKeysRight[iR].octave];  // 根据尺度确定跨度
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        for(size_t iC=0; iC<vCandidates.size(); iC++)
        {
            ...
            if(uR>=minU && uR<=maxU)
            {
                ...
                const int dist = ORBmatcher::DescriptorDistance(dL,dR);  // 计算汉明距离
        ...
        if(bestDist<thOrbDist)
        {
            ...
            const float scaleduL = round(kpL.pt.x*scaleFactor);  // 左点金字塔对应层上 u坐标
            const float scaledvL = round(kpL.pt.y*scaleFactor);  // 左点金字塔对应层上 v坐标
            const float scaleduR0 = round(uR0*scaleFactor);  // 右点在左点金字塔对应层上 u坐标
            ...
            IL = IL - IL.at<float>(w,w) *cv::Mat::ones(IL.rows,IL.cols,CV_32F);  // 去中心值

            int bestDist = INT_MAX;  // 最佳匹配距离
            int bestincR = 0;  // 最佳块匹配对应偏差
            const int L = 5;  // 滑动窗口范围 -5 ~ +5
            vector<float> vDists;  // 所有匹配距离，用于亚像素进度匹配  共 11 个匹配对象
            vDists.resize(2*L+1);
            ...
            for(int incR=-L; incR<=+L; incR++)
            {
                ...
                cv::Mat IR = mpORBextractorRight->mvImagePyramid[kpL.octave].rowRange(scaledvL-w,scaledvL+w+1).colRange(scaleduR0+incR-w,scaleduR0+incR+w+1);  // 右图图像块
                // 格式转换，去中心值
            ...
            // 抛物线极小值与 dist2 的偏差   公式源自论文   <<On Building an Accurate Stereo Matching System on Graphics Hardware>> 公式7
