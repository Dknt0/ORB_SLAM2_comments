...
{
    unique_lock<mutex> lock(mMutexConnections);
    vector<pair<int,KeyFrame*> > vPairs;  // 共视数与关键帧集   pair<int,KeyFrame*> 方便排序
...
void KeyFrame::UpdateConnections()
{
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        map<KeyFrame*,size_t> observations = pMP->GetObservations();  // 观测到地图点的观测关键帧集 <关键帧, 地图点对应关键点序号>
    ...
    /* 2.筛选共视关系，最小共视阈值为 15，若没有一个超过阈值，保留最大共视的 KF，同时更新对方共视 */
    ...
    // 这里写为 pair<int, KeyFrame*> 是为了方便直接调用排序算法，不需要定义额外仿函数
    ...
    // 填充成员变量
    {
        ...
        {
            mpParent = mvpOrderedConnectedKeyFrames.front();  // 共视数最多的关键帧作为父关键帧
...
{
    int idx = pMP->GetIndexInKeyFrame(this);  // 查询地图点在此帧内对应关键点的序号
    if(idx>=0)
        mvpMapPoints[idx]=static_cast<MapPoint*>(NULL);  // 清除关联, 赋 nullptr
}
    ...
    /* 3.删除生成树中当前帧的观测信息 */
    {
        ...
        while(!mspChildrens.empty())
        {
            ...
            for(set<KeyFrame*>::iterator sit=mspChildrens.begin(), send=mspChildrens.end(); sit!=send; sit++)
            {
                ...
                vector<KeyFrame*> vpConnected = pKF->GetVectorCovisibleKeyFrames();  // 有序共视关键帧向量
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    for(int i=0; i<N; i++)
    {
        if(mvpMapPoints[i])
        {
            ...
            float z = Rcw2.dot(x3Dw)+zcw;  // 相对于相机深度，在单目中不能直接获取深度，所以这里用了坐标变幻
            vDepths.push_back(z);
        }
    }
