...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    /* 1.选择与当前 KF 有相同视觉词汇且不构成共视关系的，作为候选 KF  填充 lKFsSharingWords */

    {
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            list<KeyFrame*> &lKFs =   mvInvertedFile[vit->first];  // 包含这个单词的关键帧列表
    ...
    list<pair<float,KeyFrame*> > lScoreAndMatch;  // 匹配分数-关键帧列表  list<pair<相似性得分, 关键帧>>
    ...
    /* 3.按照相同单词数阈值对候选关键帧进行筛选，对满足要求的候选 KF 计算相似性得分，保留得分高于阈值的关键帧  填充 lScoreAndMatch */
    ...
    /* 4.依据共视关系计算累计相似性分数，考虑候选 KF 共视关键帧中同样为候选 KF 的那部分，对他们与被搜索帧的相似性得分求和  填充 lAccScoreAndMatch */

    // 遍历匹配分数-关键帧列表
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        KeyFrame* pKFi = it->second;  // 关键帧
        vector<KeyFrame*> vpNeighs = pKFi->GetBestCovisibilityKeyFrames(10);  // 获取 10 个最佳共视关键帧
        ...
        lAccScoreAndMatch.push_back(make_pair(accScore,pBestKF));  // 注意 pBestKF 不一定是 pKFi, 这个列表中可能有相同的关键帧多次出现
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    list<pair<float,KeyFrame*> > lScoreAndMatch;  // 相似性得分-关键帧列表  list<pair<匹配分数, 关键帧>>

    int nscores=0;  // 达标关键帧数量

    // Compute similarity score.
    /* 3.按照相同单词数阈值对候选关键帧进行筛选，对满足要求的候选 KF 计算相似性得分  填充 lScoreAndMatch */
    ...
    list<pair<float,KeyFrame*> > lAccScoreAndMatch;  // 累计匹配分数-关键帧列表  list<pair<累计匹配分数, 关键帧>>
