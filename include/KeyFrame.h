...
public:
    ...
    // 关键帧数据库中用到的变量
    // 用于回环候选关键帧计算，仅在调用该函数时临时使用  使用这些变量时，此关键帧作为候选关键帧出现
    ...
    // 用于重定位候选关键帧计算，仅在调用该函数时临时使用  使用这些变量时，此关键帧作为候选关键帧出现
    long unsigned int mnLoopQuery;  // 当前被搜索关键帧的序号  检测重定位候选关键帧计算
...
protected:
    ...
    // 继承自 Frame
    std::vector< std::vector <std::vector<size_t> > > mGrid;  // 网格  (x)(y)(vector<网格内特征点索引>)
