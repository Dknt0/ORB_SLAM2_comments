...
void Optimizer::BundleAdjustment(const vector<KeyFrame*>& vpKFs,
  ...
  g2o::BlockSolver_6_3* solver_ptr =
      new g2o::BlockSolver_6_3(linearSolver);  // 块求解器
  g2o::OptimizationAlgorithmLevenberg* solver =
      new g2o::OptimizationAlgorithmLevenberg(solver_ptr);  // 图优化求解器
  optimizer.setAlgorithm(solver);
  if (pbStopFlag) optimizer.setForceStopFlag(pbStopFlag);  // 设置强制停止标志
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKF->mnId > maxKFid) maxKFid = pKF->mnId;  // 记录最大关键帧序号
  }
    ...
    for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin();
      ...
      if (pKF->mvuRight[mit->second] < 0) {
        ...
        g2o::EdgeSE3ProjectXYZ* e =
            new g2o::EdgeSE3ProjectXYZ();  // 边 空间点到成像平面

        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                            optimizer.vertex(id)));  // 观测地图点
        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                            optimizer.vertex(pKF->mnId)));  // 观测关键帧
        e->setMeasurement(obs);  // 设置观测结果
        const float& invSigma2 =
            pKF->mvInvLevelSigma2[kpUn.octave];  // 逆面积比例 <1
        e->setInformation(Eigen::Matrix2d::Identity() *
                          invSigma2);  // 信息矩阵  层数越低越大
      ...
      // 如果深度有效
        ...
        g2o::EdgeStereoSE3ProjectXYZ* e =
            new g2o::EdgeStereoSE3ProjectXYZ();  // 边 空间点到双目成像平面

        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                            optimizer.vertex(id)));  // 设置地图点
        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                            optimizer.vertex(pKF->mnId)));  // 设置关键帧
        e->setMeasurement(obs);  // 设置观测结果
        const float& invSigma2 =
            pKF->mvInvLevelSigma2[kpUn.octave];  // 逆面积比例 <1
        Eigen::Matrix3d Info = Eigen::Matrix3d::Identity() *
                               invSigma2;  // 信息矩阵  层数越低越大，与深度无关
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    g2o::VertexSE3Expmap* vSE3 = static_cast<g2o::VertexSE3Expmap*>(
        optimizer.vertex(pKF->mnId));         // 图模型关键帧节点
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    ...
    g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(
        optimizer.vertex(pMP->mnId + maxKFid + 1));  // 图模型地图点节点
      ...
      if (pMP) {
        ...
        if (pFrame->mvuRight[i] < 0) {
          ...
          g2o::EdgeSE3ProjectXYZOnlyPose* e =
              new g2o::EdgeSE3ProjectXYZOnlyPose();  // 边 空间点到平面
                                                     // Motion-only

          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(0)));  // 观测关键帧
          e->setMeasurement(obs);
          const float invSigma2 =
              pFrame->mvInvLevelSigma2[kpUn.octave];  // 逆面积比例 <1
          e->setInformation(Eigen::Matrix2d::Identity() *
                            invSigma2);  // 信息矩阵  层数越低越大
        ...
        else  // Stereo observation
        {
          ...
          e->setVertex(
              0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                     optimizer.vertex(0)));  // 边 空间点到双目平面 Motion-only
          e->setMeasurement(obs);            // 设置观测结果
          const float invSigma2 =
              pFrame->mvInvLevelSigma2[kpUn.octave];  // 逆面积比例 <1
          Eigen::Matrix3d Info =
              Eigen::Matrix3d::Identity() * invSigma2;  // 信息矩阵 层数越低越大
          e->setInformation(Info);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag,
  ...
  const vector<KeyFrame*> vNeighKFs =
  ...
  for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    vector<MapPoint*> vpMPs =
        (*lit)->GetMapPointMatches();  // 获取关键点对应的地图点观测
    // 遍历关键点
  ...
  /* 3.遍历局部 MP 观测关系，确定固定 KF。观测到局部 MP，且不是局部 KF
   * 的，为固定 KF */

  // Fixed Keyframes. Keyframes that see Local MapPoints but that are not Local
  // Keyframes 观测到局部地图点，但不是局部关键帧的，为固定关键帧
  ...
  if (pbStopFlag) optimizer.setForceStopFlag(pbStopFlag);  // 停止标志位置
  ...
  for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(),
    ...
    if (pKFi->mnId > maxKFid) maxKFid = pKFi->mnId;  // 记录最大关键帧序号
  }
  ...
  for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(),
    ...
    g2o::VertexSBAPointXYZ* vPoint =
        new g2o::VertexSBAPointXYZ();  // 地图点节点
    vPoint->setEstimate(Converter::toVector3d(pMP->GetWorldPos()));  // 初值
      ...
      if (!pKFi->isBad()) {
        ...
        if (pKFi->mvuRight[mit->second] < 0) {
          ...
          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(id)));  // 观测地图点
          e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(pKFi->mnId)));  // 观测关键帧
          e->setMeasurement(obs);  // 设置观测结果
          const float& invSigma2 =
              pKFi->mvInvLevelSigma2[kpUn.octave];  // 逆面积比例 <1
          e->setInformation(Eigen::Matrix2d::Identity() *
                            invSigma2);  // 信息矩阵  层数越低越大
        ...
        else  // Stereo observation
        {
          ...
          e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(id)));  // 设置地图点
          e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                              optimizer.vertex(pKFi->mnId)));  // 设置关键帧
          e->setMeasurement(obs);  // 设置观测结果
          const float& invSigma2 =
              pKFi->mvInvLevelSigma2[kpUn.octave];  // 逆面积比例 <1
          Eigen::Matrix3d Info =
...
void Optimizer::OptimizeEssentialGraph(
  ...
  g2o::BlockSolver_7_3::LinearSolverType* linearSolver =
      new g2o::LinearSolverEigen<
          g2o::BlockSolver_7_3::PoseMatrixType>();  // 线性求解器
  g2o::BlockSolver_7_3* solver_ptr =
      new g2o::BlockSolver_7_3(linearSolver);  // 块求解器
  g2o::OptimizationAlgorithmLevenberg* solver =
      new g2o::OptimizationAlgorithmLevenberg(solver_ptr);  // 图优化求解器
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    LoopClosing::KeyFrameAndPose::const_iterator it =
        CorrectedSim3.find(pKF);  // 在修正位姿映射中查询当前关键帧
  ...
  set<pair<long unsigned int, long unsigned int> >
      sInsertedEdges;  // 回环边两端的节点 <关键帧序号, 关键帧序号>

  const Eigen::Matrix<double, 7, 7> matLambda =
      Eigen::Matrix<double, 7, 7>::Identity();  // Lambda 矩阵, 作为信息矩阵
  ...
  for (map<KeyFrame*, set<KeyFrame*> >::const_iterator
    ...
    for (set<KeyFrame*>::const_iterator sit = spConnections.begin(),
      ...
      sInsertedEdges.insert(
          make_pair(min(nIDi, nIDj), max(nIDi, nIDj)));  // 记录边的连接关系
    }
  }
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    LoopClosing::KeyFrameAndPose::const_iterator iti =
        NonCorrectedSim3.find(pKF);  // 在未修正位姿映射中查询当前关键帧
    ...
    if (pParentKF) {
      ...
      LoopClosing::KeyFrameAndPose::const_iterator itj =
          NonCorrectedSim3.find(pParentKF);  // 在未修正位姿映射中查询父关键帧
    ...
    for (set<KeyFrame*>::const_iterator sit = sLoopEdges.begin(),
      ...
      if (pLKF->mnId < pKF->mnId) {
        ...
        LoopClosing::KeyFrameAndPose::const_iterator itl =
            NonCorrectedSim3.find(pLKF);  // 在未修正位姿映射中查询父关键帧
    ...
    const vector<KeyFrame*> vpConnectedKFs =
        pKF->GetCovisiblesByWeight(minFeat);  // 超过阈值的共视图边
    // 遍历共视图边
    for (vector<KeyFrame*>::const_iterator vit = vpConnectedKFs.begin();
         vit != vpConnectedKFs.end(); vit++) {
      KeyFrame* pKFn = *vit;  // 共视关键帧
      // pKFn有效 且 pKFn不是父关键帧 且 pKFn不是子关键帧 且 pKFn不构成回环边
      // 即，pKFn 与当前帧的共视关系不属于生成树与回环边
      if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) &&
        ...
        if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
          ...
          LoopClosing::KeyFrameAndPose::const_iterator itn =
              NonCorrectedSim3.find(pKFn);  // 在未修正位姿映射中查询父关键帧
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    g2o::VertexSim3Expmap* VSim3 =
        static_cast<g2o::VertexSim3Expmap*>(optimizer.vertex(nIDi));  // 节点
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
    ...
    Eigen::Matrix<double, 3, 1> eigCorrectedP3Dw =
        correctedSwr.map(Srw.map(eigP3Dw));  // 修正后世界坐标
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
  ...
  g2o::BlockSolverX* solver_ptr =
      new g2o::BlockSolverX(linearSolver);  // 块求解器
  g2o::OptimizationAlgorithmLevenberg* solver =
      new g2o::OptimizationAlgorithmLevenberg(solver_ptr);  // 图优化求解器
  optimizer.setAlgorithm(solver);
  ...
  const vector<MapPoint*> vpMapPoints1 =
      pKF1->GetMapPointMatches();              // 关键帧 1 地图点匹配
  ...
  for (int i = 0; i < N; i++) {
    ...
    const int i2 =
        pMP2->GetIndexInKeyFrame(pKF2);  // pMP2 在 pKF2 中的关键点序号

    // 如果 pMP1 和 pMP2 都有效
    if (pMP1 && pMP2) {
      // 如果 pMP1 和 pMP2 不为坏点，且存在 pKF2 对 pMP2 的观测
      if (!pMP1->isBad() && !pMP2->isBad() && i2 >= 0) {
        g2o::VertexSBAPointXYZ* vPoint1 =
            new g2o::VertexSBAPointXYZ();  // MP1 节点
